...
* **打包 / 構建系統**：
    ...
    * 新增精簡 C++ 源碼包發佈流程（`release-source.yml`）：以 `OPENCC_SOURCE_PACKAGE_PROFILE=opencc` 產生含 C++ 核心與 Jieba 插件的 CPack 壓縮包（不含 Node.js、Python、文檔與 packaging），並以 CMake 與 Bazel 雙路徑冒煙測試（含 Jieba 插件建置）後，重新打包以正規化檔案擁有者、時間戳與排序，產生 `.tar.gz` 與 `.tar.xz` 上傳至 draft release；為此保留 Bazel workspace 檔案與頂層 `patches/`，僅排除可能含 API key 的 `.bazelrc.user`。
