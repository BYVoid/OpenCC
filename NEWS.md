...
* **小篆對照（實驗性）**：
    * 新增 Unicode 18.0 篆書區塊（Seal，U+3D000..U+3FC3F，共 11,328 字）與現代漢字的雙向對照。`SealCharacters.txt`（小篆 → 漢字）取自 UCD `SealSources.txt` 的 `kSEAL_MCJK` 屬性，反向詞典 `SealCharactersRev`（漢字 → 小篆）與 `TWVariantsRev` 等一樣於建置時生成。一個漢字對應多個小篆時（共 94 字），反向預設取碼位最小者（通常為《說文解字》正篆，重文在後），另有 4 字以 `@reverse-prefer` 改取版本來源（藤花榭本、陳昌治本、祁寯藻本《說文解字繫傳》、段注本）更完整者。
