...
if(GIT_FOUND)
  ...
  if(_git_result EQUAL 0 AND _git_raw)
    ...
    if(_match_tag)
      ...
      set(OPENCC_VERSION_MINOR "${CMAKE_MATCH_2}")
      set(OPENCC_VERSION_REVISION "${CMAKE_MATCH_3}")
      set(OPENCC_VERSION_SUFFIX "${_dirty_suffix}")
      set(OPENCC_VERSION_IS_RELEASE TRUE)
      set(_git_version_computed TRUE)
    endif()

    # Case 2: Dev build — v1.2.3-N-g<sha>  (N > 0)
    if(NOT _git_version_computed)
      string(REGEX MATCH "^v([0-9]+)\\.([0-9]+)\\.([0-9]+)-([0-9]+)-g([0-9a-f]+)$" _match_dev "${_git_raw}")
      if(_match_dev)
        set(OPENCC_VERSION_MAJOR "${CMAKE_MATCH_1}")
        set(OPENCC_VERSION_MINOR "${CMAKE_MATCH_2}")
        set(OPENCC_VERSION_REVISION "${CMAKE_MATCH_3}")
        set(OPENCC_VERSION_SUFFIX ".dev${CMAKE_MATCH_4}+g${CMAKE_MATCH_5}${_dirty_suffix}")
