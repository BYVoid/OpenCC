// Bazel build entry-point for the OpenCC Node.js native addon.
//
// Unlike node/opencc.cc (which uses unity-build #includes of .cpp files for
// faster node-gyp compilation), this file contains only the N-API binding
// code and relies on //src:opencc being compiled as a proper cc_library by
// Bazel.  The two files share the same logic; keep them in sync.
