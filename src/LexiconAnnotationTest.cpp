...
TEST_F(LexiconAnnotationTest, DefaultBehaviorIgnoresComments) {
  ...
  // Default behavior (preserveComments=false) should work
  FILE* readFp = fopen(testFileName.c_str(), "r");
  const TextDictPtr& dict = TextDict::NewFromFile(readFp);
  fclose(readFp);

  EXPECT_EQ(dict->GetLexicon()->Length(), 2);
  EXPECT_FALSE(dict->GetLexicon()->HasAnnotations());
}

TEST_F(LexiconAnnotationTest, DefaultBehaviorRejectsCommentLines) {
  FILE* fp = fopen(testFileName.c_str(), "w");
  fprintf(fp, "# This is a comment\n");
  fprintf(fp, "A\tB\n");
  fclose(fp);

  // Default behavior should throw exception on comment lines (original behavior)
  FILE* readFp = fopen(testFileName.c_str(), "r");
  EXPECT_THROW(TextDict::NewFromFile(readFp), InvalidTextDictionary);
  fclose(readFp);
}
